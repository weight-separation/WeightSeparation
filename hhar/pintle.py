...
import_name = dir_name + '.hhar_data'
...
def v_num_of_neurons_per_layer():
	return [[20, 120, 1], [9, 59, 8], [3, 28, 32], [1, 13, 64], [60], [6]]
...
def v_train_input_variables():
	data = __import__(import_name).hhar_data
	train_set = data.train_set()
	train_image_reshaped = np.reshape(train_set[0], ([-1, 20, 120, 1]))
	return [[train_image_reshaped, train_set[1]], 1.0, 1.0]

def v_test_input_variables():
	data = __import__(import_name).hhar_data
	test_set = data.test_set()
	test_image_reshaped = np.reshape(test_set[0], ([-1, 20, 120, 1]))
...
def v_execute(graph, sess, input_tensors, input_variables, output_tensors=None,
	...
	if ground_truth is not None:
		...
		print("hhar inference accuracy: %f" % test_accuracy)
