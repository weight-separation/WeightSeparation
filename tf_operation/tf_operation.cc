...
class GetWeightAddressOp : public OpKernel {
	public:
		...
		void Compute(OpKernelContext* context) override {
			...
			OP_REQUIRES_OK(context, context->allocate_output(0,
				TensorShape({ref_inputs.size()}), &output_tensor1, alloc_attr));
			...
			OP_REQUIRES_OK(context, context->allocate_output(1,
				TensorShape({ref_inputs.size()}), &output_tensor2, alloc_attr));
			...
			for (int i = 0; i < ref_inputs.size(); i++) {
				...
				output1.data()[i] = (int64)input.data();
				output2.data()[i] = input.size();
			}
		}
};
...
REGISTER_KERNEL_BUILDER(Name("GetWeightAddress").Device(DEVICE_GPU).HostMemory("weight_address").HostMemory("weight_len"), GetWeightAddressOp);
